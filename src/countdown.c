      ...
      if (!timer_running) {                                          // Start the timer
        ...
        if (fullstop) {
          ...
          if (per_last_yday != yday) {    
            ...
            while ( (yday - day_gap) > per_last_yday ){
...
static void handle_second_tick(struct tm *t, TimeUnits units_changed) {
  ...
  if (timer_running) {
    if (decrement_timer()) {
      ...
      dailyRegisters[per_last_wday].yday = per_last_yday;
      dailyRegisters[per_last_wday].wday = per_last_wday;
...
void handle_deinit() {
  ...
  persist_write_int(PERSIST_LAST_WDATE, per_last_wday);
  APP_LOG(APP_LOG_LEVEL_DEBUG, "per_last_wday %d ", per_last_wday);
