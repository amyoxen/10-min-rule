...
void chart_layer_set_data(ChartLayer* layer, 
  ...
  if (layer) {
    ...
    pData->pZOrigData = (float*) malloc(pData->iNumOrigPoints * sizeof(float));
    ...
    if (typeZ == eINT) {
      // cast
      for (unsigned int i = 0; i < iNumPoints; ++i)
	pData->pZOrigData[i] = (float)(((int*)pZ)[i]);
    }
    else if (typeZ == eFLOAT) {
      memcpy(pData->pZOrigData, pZ, iNumPoints * sizeof(float));
    }
...
static void chart_layer_update_layout(ChartLayer* layer) {
  if (layer) {
	...
	qsort(sort_order, pData->iNumOrigPoints, sizeof(ChartSortHelper), &cmpChartSortHelper);
      ...
      pData->pZData = (int*)malloc(pData->iNumPoints * sizeof(int));
      // figure out Y-scale
  ...
  if (pData->pZOrigData[i] > fMaxY)
    fMaxY = pData->pZOrigData[i];
	if (pData->pYOrigData[i] < fMinY)
	  fMinY = pData->pYOrigData[i];
  if (pData->pZOrigData[i] < fMinY)
	  fMinY = pData->pZOrigData[i];
      }
  ...
  pData->pZData[j] = bounds.size.h - ((int)(fYScale * (pData->pZOrigData[sort_order[i].index] - fMinY)) + pData->iMargin);     
      }
...
//    const bool bShowPoints = (data->typePlot != eBAR) && ((data->typePlot == eSCATTER) || (data->bShowPoints && (data->iNumOrigPoints < ((unsigned int)bounds.size.w / 3))));
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "drawing y lines...");
  ...
  graphics_draw_line(ctx, 
   ...
   APP_LOG(APP_LOG_LEVEL_DEBUG, "drawing z lines...");
      }
      //else if (data->typePlot == eBAR) {
/*	graphics_fill_rect(ctx,
			   ((GRect) {
			     .origin = { data->pXData[i] - (data->iBarWidth / 2), data->pZData[i] },
			       .size = { data->iBarWidth, (((data->iYAxisIntercept > (bounds.size.h - data->iMargin)) ? (bounds.size.h - data->iMargin) : data->iYAxisIntercept) - data->pZData[i]) } }),
			   0,
			   GCornersAll);
      //}
*/
...
//      if (bShowPoints) {
  ...
  graphics_draw_circle(ctx, 
			     ((GPoint) {
			       .x = data->pXData[i],
				 .y = data->pZData[i] } ), iPointRadius);
//      }
    
    APP_LOG(APP_LOG_LEVEL_DEBUG, "%d",(int) data->pZData[i]);
    APP_LOG(APP_LOG_LEVEL_DEBUG, "%d",data->pXData[i]);
    APP_LOG(APP_LOG_LEVEL_DEBUG, "%d",data->pYData[i]);
    }
  }
}
